...
export default function BillsScreen() {
  ...
  const { receiptImagePrintBridge, startReceiptImagePrintJob } =
    useReceiptImagePrintJob();
      ...
      headerRight: () => (
        <View className="flex-row items-center">
          <TouchableOpacity
            ...
            className="mr-4 p-2 bg-zinc-100 rounded-full"
          >
  ...
  const handlePrint = async (item: any) => {
    if (!preferredPrinter) {
      Alert.alert(
        "No Printer",
        "Please select a printer in the Delivery screen first.",
      );
      return;
    }
    ...
    const closingBalance =
      item.closing_balance != null ? item.closing_balance : currentBalance;
    ...
    const cylinderBalances = item.items
      ...
      .map((bi: any) => {
        ...
        return {
          name:
            bi.item?.name ||
            itemsCatalog?.find((cat: any) => cat.id === bi.item_id)?.name ||
            "Item",
  ...
  return (
    <View className="flex-1 bg-zinc-100">
      <View className="flex-row bg-white p-2 mb-2">
        ...
        >
          <Text
            className={`font-semibold ${activeTab === "SALES" ? "text-white" : "text-zinc-500"}`}
          >
            Sales Bills
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text
            className={`font-semibold ${activeTab === "COLLECTIONS" ? "text-white" : "text-zinc-500"}`}
          >
            Collection Bills
          </Text>
        </TouchableOpacity>
