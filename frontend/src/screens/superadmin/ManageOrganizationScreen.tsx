...
export default function ManageOrganizationScreen({ route, navigation }: Props) {
  ...
  const [editMaxUsers, setEditMaxUsers] = useState(
    org.max_users?.toString() || "10",
  );
  const [address, setAddress] = useState(org.address || "");
  const [phone, setPhone] = useState(org.phone || "");
  const [salesPrefix, setSalesPrefix] = useState(
    org.bill_prefix_sales || "SHA",
  );
  const [collectionPrefix, setCollectionPrefix] = useState(
    org.bill_prefix_collection || "PAY",
  );
      ...
      <ScrollView className="flex-1 px-6 py-6">
        ...
        <View className="bg-white rounded-3xl p-6 border border-zinc-100 shadow-sm mb-6">
          <View className="flex-row items-center mb-6 border-b border-zinc-100 pb-4">
            ...
            <Text className="text-lg font-bold text-slate-900 ml-3">
              Add Admin Account
            </Text>
          </View>
            ...
            users.map((user) => (
              ...
              >
                ...
                <View className="flex-row items-center">
                  <View
                    className={`px-2 py-1 rounded-md mr-3 ${user.is_active ? "bg-emerald-100" : "bg-red-100"}`}
                  >
                    <Text
                      className={`text-xs font-bold ${user.is_active ? "text-emerald-700" : "text-red-700"}`}
                    >
        ...
        <View className="bg-red-50 rounded-3xl p-6 border border-red-100 mb-12">
          ...
          <Text className="text-sm text-red-700 mb-4">
            Deleting this organization will permanently erase all data, entries,
            buyers, and user accounts associated with it.
          </Text>
